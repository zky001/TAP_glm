...
from language_models import GPT, GLM


def load_evaluator(args):
    if "gpt" in args.evaluator_model:
        return GPTEvaluator(args)
    elif "glm" in args.evaluator_model:
        return GLMEvaluator(args)
...
class GLMEvaluator(EvaluatorBase):
    def __init__(self, args):
        super(GLMEvaluator, self).__init__(args)
        self.evaluator_model = GLM(model_name=self.evaluator_name)
